...
void Tablero2::mostrarMenuEnVentana() {
...
void Tablero2::renderizarTexto(const std::string& texto, float x, float y, void* fuente) {
...
void Tablero2::cerrarAplicacion() {
    std::cout << "Saliendo del juego..." << std::endl;
    //glutLeaveMainLoop(); // Cierra la ventana y termina el bucle principal de GLUT
    exit(0); // Termina el programa
}

void Tablero2::manejarEntradaMenu(unsigned char key, int x, int y) {
    switch (key) {
    case '1':
...
void Tablero2::mostrarInstruccionesEnVentana() {
    glClear(GL_COLOR_BUFFER_BIT); // Limpiar la ventana
...
void Tablero2::iniciarPartida() {
    std::cout << "Iniciando partida..." << std::endl;
    inicializa();
    dibuja();
}

void Tablero2::inicializa() {
    // Primer jugador 
